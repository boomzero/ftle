...
describe("post visibility", () => {
  describe("draft posts", () => {
    it("GET /:slug returns 404 for a draft post", async () => {
      ...
      const res = await app.request("/draft-post", {}, env);
      expect(res.status).toBe(404);
    });

    it("GET / excludes draft posts", async () => {
      await createPost(env.DB, {
        slug: "listed-post",
        title: "Listed",
        source: "listed",
    ...
    it("GET /tag/:tag excludes draft posts", async () => {
      await createPost(env.DB, {
        slug: "listed-post",
        title: "Listed",
        source: "listed",
    ...
    it("GET /rss.xml excludes draft posts", async () => {
      await createPost(env.DB, {
        slug: "listed-post",
        title: "Listed",
        source: "listed",
    ...
    it("GET /sitemap.xml excludes draft posts", async () => {
      await createPost(env.DB, {
        slug: "listed-post",
        title: "Listed",
        source: "listed",
  ...
  describe("unlisted posts", () => {
    ...
    it("POST /admin/set-status/:id updates a post's status", async () => {
      ...
      // Change to unlisted via set-status
      const headers = await authedHeaders();
      res = await app.request(
        `/admin/set-status/${post.id}`,
        {
          method: "POST",
          headers: { ...headers, "Content-Type": "application/x-www-form-urlencoded" },
          body: formBody({ status: "unlisted" }),
        },
        env,
      );
    ...
    it("POST /admin/set-status/:id can change a post to listed", async () => {
      const post = await createPost(env.DB, {
        slug: "toggle-me",
        title: "Toggle Me",
        source: "test",
      ...
      // Change to listed via set-status
      const headers = await authedHeaders();
    ...
    it("POST /admin/set-status/:id returns 404 for nonexistent post", async () => {
      const headers = await authedHeaders();
      const res = await app.request(
        "/admin/set-status/99999",
        {
          method: "POST",
          headers: { ...headers, "Content-Type": "application/x-www-form-urlencoded" },
          body: formBody({ status: "unlisted" }),
    ...
    it("GET /admin shows posts regardless of status", async () => {
      await createPost(env.DB, {
        slug: "draft-post",
        title: "Draft Post",
        source: "draft",
